...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze open-access papers with GROBID."
    )
    parser.add_argument(
        "--papers-dir",
        default="papers",
    ...
    parser.add_argument(
        ...
        type=int,
        default=120,
        help="Timeout in seconds for GROBID requests"
    )
...
def extract_links(soup: BeautifulSoup) -> list[str]:
    ...
    # 1) Try TEI ref targets first
    for ref in soup.find_all("ref"):
        target = ref.get("target")
        if target and target.startswith(("http://", "https://")):
            links.add(target.rstrip(").,;]"))

    # 2) Fallback: regex from whole text
    text = soup.get_text(" ")
...
def main():
    ...
    for pdf_path in pdf_files:
        ...
        try:
            ...
            links_per_paper[pdf_path.name] = extract_links(soup)
